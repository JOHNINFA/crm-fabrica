...
export const simpleStorage = {
  ...
  async setItem(key, data) {
    try {
      ...
      // 🚫 DESHABILITADO: El guardado en backend se hace con cargueRealtimeService
      // que usa las URLs correctas (cargue-id1, cargue-id2, etc.)
      // Las peticiones a /api/cargues/ dan 404 y causan delays
  ...
  async getItem(key) {
    try {
      ...
      // 🚫 DESHABILITADO: No buscar en backend porque las URLs /api/cargues/ no existen
      // El sistema usa cargueRealtimeService que usa las URLs correctas
      // Solo retornamos null si no hay datos en localStorage
      return null;
