...
/**
 * Componente para gestionar productos directamente en localStorage
 * Esta versión evita la sincronización con el backend que causa problemas
 */
const ProductManagerDirect = ({ show, onHide }) => {
  ...
  // Cargar productos desde localStorage y aplicar filtro de productos eliminados
  useEffect(() => {
    const loadProducts = () => {
      try {
        // Cargar lista de IDs de productos eliminados
        ...
        // Cargar productos y filtrar los eliminados
        const savedProducts = localStorage.getItem('products');
        if (savedProducts) {
          const parsedProducts = JSON.parse(savedProducts);
          const filteredProducts = parsedProducts.filter(p => !deletedIds.includes(p.id));
  ...
  const saveProductsToLocalStorage = (productsToSave) => {
    try {
      ...
      const allProducts = savedProductsStr ? JSON.parse(savedProductsStr) : [];
      
      // Reemplazar los productos actuales con los nuevos
  ...
  const handleDeleteSelected = () => {
    ...
    try {
      ...
      // Actualizar lista de IDs eliminados
  ...
  // Limpiar lista de productos eliminados
  const handleResetDeletedList = () => {
    try {
      localStorage.removeItem('deletedProductIds');
      setDeletedProductIds([]);
      setMessage({ text: 'Lista de productos eliminados reiniciada', type: 'success' });
      setTimeout(() => setMessage({ text: '', type: '' }), 3000);
    } catch (error) {
      console.error('Error al reiniciar lista de eliminados:', error);
      setMessage({ text: 'Error al reiniciar lista', type: 'danger' });
    }
  };
        ...
        {deletedProductIds.length > 0 && (
          <div className="mt-3">
            <Button 
              ...
              size="sm"
              onClick={handleResetDeletedList}
            >
              Reiniciar lista de productos eliminados ({deletedProductIds.length})
            </Button>
          </div>
        )}
      </Modal.Body>
...
export default ProductManagerDirect;